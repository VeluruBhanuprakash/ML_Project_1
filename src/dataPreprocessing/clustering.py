...
class KMeansClustering:
    ...
    def elbow_plot(self,data):
        ...
        try:
            ...
            plt.title('The elblow plot')
            plt.xlabel('Number of clusters')
            plt.ylabel('wcss')
            plt.savefig("preprocessing_data/K-Means_Elbow.PNG")

            self.knn =knee_locator(range(1,11),wcss,curve='convex',direction='decreasing')
            self.logger_object.logger(self.file_object, 'The optimum number of clusters is: '+str(self.kn.knee)+' . Exited the elbow_plot method of the KMeansClustering class')
            return self.knn
    ...
    def create_cluster(self,data,number_of_clusters):
        self.logger_object.logger(self.file_object, 'Entered the create_clusters method of the KMeansClustering class')
        self.data =data
        try:
            ...
            self.logger_object.logger(self.file_object, 'succesfully created '+str(self.kn.knee)+ 'clusters. Exited the create_clusters method of the KMeansClustering class')
            return self.data
        except Exception as e:
            pass
